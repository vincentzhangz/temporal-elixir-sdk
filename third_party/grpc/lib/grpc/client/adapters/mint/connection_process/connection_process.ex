if Code.ensure_loaded?(Mint.HTTP) do
  defmodule GRPC.Client.Adapters.Mint.ConnectionProcess do
    ...
    def init({scheme, host, port, opts}) do
      ...
      {keepalive, opts} = Keyword.pop(opts, :keepalive, nil)
      ...
      case Mint.HTTP.connect(scheme, host, port, opts) do
        {:ok, conn} ->
          ...
          state = State.new(conn, state_opts)
          state = schedule_keepalive_ping(state)
          {:ok, state}
    ...
    def handle_info(:keepalive_ping, state) do
      if State.keepalive_enabled?(state) do
        case Mint.HTTP2.ping(state.conn, @keepalive_payload) do
          {:ok, conn, ref} ->
            ack_timer = Process.send_after(self(), {:keepalive_ack_timeout, ref}, state.keepalive.timeout)

            state
            |> State.update_conn(conn)
            |> State.put_keepalive_ping(ref, ack_timer)
            |> then(&{:noreply, &1})

          {:error, conn, reason} ->
            Logger.warning(
              "keepalive ping to #{state.scheme}://#{state.host}:#{state.port} failed: #{inspect(reason)}"
            )

            check_connection_status(State.update_conn(state, conn))
        end
      else
        {:noreply, state}
      end
    ...
    def handle_info({:keepalive_ack_timeout, ref}, state) do
      if State.keepalive_matching_ping?(state, ref) do
        Logger.warning(
          "keepalive ping not acknowledged within #{state.keepalive.timeout}ms; " <>
            "considering connection to #{state.scheme}://#{state.host}:#{state.port} dead"
        )

        state
        |> State.clear_keepalive_ping()
    ...
    def handle_info(message, state) do
      case Mint.HTTP.stream(state.conn, message) do
        ...
        {:ok, conn, responses} ->
          ...
          state = handle_keepalive_pongs(responses, state)
    ...
    defp finish_all_pending_requests(state) do
      ...
      clean_state = State.clear_keepalive_timers(clean_state)
    ...
    defp attempt_reconnect(state) do
      ...
      case Mint.HTTP.connect(state.scheme, state.host, state.port, state.connect_opts) do
        {:ok, conn} ->
          ...
          {:noreply, schedule_keepalive_ping(new_state)}
    ...
    defp schedule_keepalive_ping(state) do
      if State.keepalive_enabled?(state) do
        timer = Process.send_after(self(), :keepalive_ping, state.keepalive.interval)
        State.set_keepalive_ping_timer(state, timer)
      else
        state
      end
    end

    defp handle_keepalive_pongs(responses, state) do
      if State.keepalive_enabled?(state) do
        case Enum.find(responses, &match?({:pong, _ref}, &1)) do
          {:pong, ref} ->
            if State.keepalive_matching_ping?(state, ref) do
              state
              |> State.clear_keepalive_ping()
              |> schedule_keepalive_ping()
            else
              state
            end
